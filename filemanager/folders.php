                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    if (!empty($name)) {
                        ...
                        if (isset($assign) && !empty($assign)) {
                            ...
                            $qry         = mysqli_query($mysqli,"SELECT upload_dirs, id FROM cbt_user ORDER BY id");
                            while ($row = mysqli_fetch_row($qry)) {
                                $x         = $row[0] . $folder_name;
                                $x2        = explode(",", $x);
                                $assignDir = implode(",", $x2);
                                $qry2      = mysqli_query($mysqli,"UPDATE cbt_user SET upload_dirs = '" . $assignDir . "' WHERE id={$row[1]}");
                            }
                        }
                        //}
                        $name = "";
