...
if($level=="user"){
    ...
    if(!stristr($access,"i")){  //if user can't see all files - show only his files in his assigned folders
        ...
        if(stristr($access,"t")){ //if user can view only his uploaded files
            ...
            $sJoin=" INNER JOIN {$db_pr}folders fo ON f.catID=fo.id
                    INNER JOIN cbt_user u ON f.userID=u.id ";
    ...
    while ( $aRow = mysqli_fetch_array( $aRowesult ) )
    {
		...
		//$ul = $_SERVER['SERVER_NAME'].($_SERVER['SERVER_PORT']!=''?":".$_SERVER['SERVER_PORT']:"");
        $ul = "hssepass.pgn.co.id";
        if(strtolower($aRow["extension"])=='pdf'){
            $editable .="<center><a class=\"iframe\" href=\"javascript:viewPdf('https://{$ul}{$script_dir}{$aRow["path"]}');\" title=\"Preview\"><span class=\"glyphicon glyphicon-zoom-in\"></span></a>&nbsp;</center>";
        }

        if(in_array(strtolower($aRow["extension"]),$docs)){
            $editable .="<a class=\"iframe\" href=\"javascript:viewDoc('https://{$_SERVER['SERVER_NAME']}{$script_dir}{$aRow["path"]}');\" title=\"Preview\"><span class=\"glyphicon glyphicon-zoom-in\"></span></a>&nbsp;";
