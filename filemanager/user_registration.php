...
if ($allow_reg) {
    ...
    if (!empty($_REQUEST["register"]) && $_REQUEST['register'] == "yes") {
        if ($username == "" || $password == "" || $email == "") {
            ...
            $sSQL = "SELECT * FROM `cbt_user` WHERE `username`='" . $username . "' OR email='" . $email . "'";
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                if ($auto_approve) {
                    ...
                    $query      = "INSERT INTO cbt_user (dateCreated,username,password,email,extensions,quota,filesize,active,accesslevel,upload_dirs)
                    VALUES ('" . date("Y-m-d H:i:s") . "','" . $username . "','" . md5($password) . "','" . $email . "','{$extensions}','{$quota}','{$filesize}','" . $active . "','ghk','{$upload_dir}')";
                } else {
                    $active = "2";
                    $ap     = "No";
                    $query  = "INSERT INTO cbt_user (dateCreated,username,password,email,extensions,quota,filesize,active,accesslevel,upload_dir)
                ...
                if (mysqli_query($mysqli,$query)) {
                    ...
                    if ($auto_approve) {
                        ...
                        $sSQL                    = "UPDATE `cbt_user` SET `last_login`=NOW() WHERE `id`='" . $newID . "'";
                    ...
                    if ($auto_create_user_folder) {
                        ...
                        if (!empty($name)) {
                            ...
                            $sql = "UPDATE cbt_user SET upload_dirs='{$folderID}' WHERE id='$newID'";
                            $result = mysqli_query($mysqli,$sql) or die("Error occurred - tried  to insert new folder.");
                        }
                    }
                }
