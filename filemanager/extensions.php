                    ...
                    if (!empty($name)) {
                        if (($name == "EXE" || $name == "COM" || $name == "BAT") && $demo) {
                            ...
                            if (isset($assign) && !empty($assign)) {
                                $extname = "," . $name;
                                $qry = mysqli_query($mysqli,"SELECT extensions, id FROM cbt_user ORDER BY id");
                                while ($row = mysqli_fetch_row($qry)) {
                                    $x = $row[0] . $extname;
                                    $x2 = explode(",", ltrim($x, ','));
                                    sort($x2);
                                    $assignExt = implode(",", $x2);
                                    $qry2 = mysqli_query($mysqli,"UPDATE cbt_user SET extensions = '" . $assignExt . "' WHERE id={$row[1]}");
                                }
                            }
                        }
                        $name = "";
            ...
            if (is_array($_POST['extToDel'])) {
                if (join(",", $_POST['extToDel']) != '') {
                    ...
                    foreach ($_POST['extToDel'] AS $val) {
                        ...
                        $sql = "SELECT * FROM cbt_user WHERE extensions LIKE '%{$ext['name']}%' ";
                        $res = mysqli_query($mysqli,$sql);
                        while ($row = mysqli_fetch_assoc($res)) {
                            $usrExt = explode(',', $row['extensions']);
                            unset($usrExt[$ext['name']]);
                            mysqli_query($mysqli,"UPDATE cbt_user SET extensions ='" . implode(',', $usrExt) . "' WHERE id='{$row['id']}'");
                        }
                    }
                }
            }
