...
if (isset($_SESSION["logged_in"]) && $_SESSION["logged_in"] != true) {
    ...
    if ($level == "admin") {
        ...
        if($demo){
            ...
            if (!empty($username) && !empty($email)) {
                ...
                if ($demo) {
                    ...
                    $sql = "UPDATE cbt_user SET username='" . $username . "',email='" . $email . "',accesslevel='" . $accesslevelStr . "',extensions='" . $extensionsStr . "',quota='" . $quota . "',filesize='" . $filesize . "',upload_dir='" . $upload_dir . "',upload_dirs='" . $foldStr . "',active='" . $active . "' WHERE id='" . $id . "'";
                    $result = mysqli_query($mysqli,$sql) or die("Error occurred - tried  to update user.");
                }
                if ($id == "1") {
                    $sql = "UPDATE cbt_user SET accesslevel='abcdefghijklmnopqrstuvwxyz',active='1' WHERE id='1'";
                    $result = mysqli_query($mysqli,$sql) or die("Error occurred - tried  to update user.");
                }
                if (!empty($password)) {
                    if ($demo) {
                        ...
                        $sql = "UPDATE cbt_user SET password='" . md5($password) . "' WHERE id='" . $id . "'";
                ...
                if ($admin) {
                    $sql = "UPDATE cbt_user SET accesslevel='abcdefghijklmnopqrstuvwxyz' WHERE id='" . $id . "'";
                    $result = mysqli_query($mysqli,$sql) or die("Error occurred - tried  to update user.");
        ...
        $sSQL = "SELECT id,username,email,filesize,quota,extensions,accesslevel as accesslevel2,upload_dir,upload_dirs,active FROM cbt_user WHERE id='" . $id . "'";
