...
if (!empty($_REQUEST["token"])) {
	...
	$token = base64_decode(base64_decode($_REQUEST["token"]));
	$tkn = explode("##",$token);
	$pem = explode(":",$tkn[0]);
	$pem = $pem[1];
	$username = $tkn[1];
	...
	$pem2 = explode(":",$tkn[3]);
	$pem2 = $pem2[1];
	#var_dump($pem,$pem2,date('i'));exit;
	if(!$_SESSION['username'] && !$_SESSION['logged_in']){
		if($pem==$pem2 && $pem2==date('i')){
			$sSQL = "SELECT * FROM `cbt_user` WHERE `username`='" . $username . "' AND active='1'";
			...
			if (mysqli_num_rows($result) > 0) {
				...
				if ($password != $row["password"]) {
					...
					$sSQL                    = "UPDATE `cbt_user` SET `last_login`=NOW() WHERE `id`='" . $row["id"] . "'";
