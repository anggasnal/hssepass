...
class Cbt_tes_user_model extends CI_Model{
    ...
    function update_menit($tesid, $waktu){
        $sql = 'UPDATE cbt_tes_user SET tesuser_creation_time=TIMESTAMPADD(MINUTE, '.$waktu.', tesuser_creation_time) WHERE tesuser_tes_id="'.$tesid.'"';
        $this->db->query($sql);
    }
     ...
     *
     * @param      string  $tesid  The tesuser identifier
     *
     * @return     <type>  Number of by status waktu.
     */
    function count_by_status_waktu($tesid){
        $this->db->select('COUNT(tes_id) AS hasil')
                 ->where('(tes_id="'.$tesid.'" AND tesuser_status="1" AND TIMESTAMPADD(MINUTE, tes_duration_time, tesuser_creation_time)>NOW())')
                 ->from($this->table)
     ...
     * @return     <type>  Number of by status waktu.
     */
    function count_by_status_waktuuser($tesid, $waktuuser){
        $this->db->select('COUNT(tes_id) AS hasil')
                 ->where('(tes_id="'.$tesid.'" AND tesuser_status="1" AND TIMESTAMPADD(MINUTE, tes_duration_time, tesuser_creation_time)>"'.$waktuuser.'")')
                 ->from($this->table)
                 ->join('cbt_tes', 'cbt_tes_user.tesuser_tes_id = cbt_tes.tes_id');
        return $this->db->get();
    }

    function get_by_user_status($id){
        $this->db->where('tesuser_user_id="'.$id.'" AND tesuser_status!=4')
                 ->from($this->table)
                 ->join('cbt_tes', 'cbt_tes_user.tesuser_tes_id = cbt_tes.tes_id');
        return $this->db->get();
    }

    function get_by_user_tes_limit($id, $tes_id){
        $this->db->where('tesuser_user_id="'.$id.'" AND tesuser_tes_id="'.$tes_id.'" AND tesuser_status=1')
                 ->from($this->table)
                 ->join('cbt_tes', 'cbt_tes_user.tesuser_tes_id = cbt_tes.tes_id')
                 ->limit(1);
        return $this->db->get();
    ...
    function count_by_user_tes($id, $tes_id){
        $this->db->select('COUNT(*) AS hasil')
                 ->where('tesuser_user_id="'.$id.'" AND tesuser_tes_id="'.$tes_id.'"')
                 ->from($this->table);
        return $this->db->get();
    }

    function count_by_user_tes_selesai($id, $tes_id){
        $this->db->select('COUNT(*) AS hasil')
                 ->where('tesuser_user_id="'.$id.'" AND tesuser_tes_id="'.$tes_id.'" AND tesuser_status=4')
                 ->from($this->table);
        return $this->db->get();
    }
	
    function get_by_user_tes($id, $tes_id){
        $this->db->where('tesuser_user_id="'.$id.'" AND tesuser_tes_id="'.$tes_id.'"')
                 ->from($this->table)
                 ->limit(1);
        return $this->db->get();
    }
    ...
    function get_by_tes_group_urut_tanggal($tes_id, $grup_id, $urutkan, $tanggal){
        ...
        $this->db->select('cbt_tes_user.*, cbt_tes.*, cbt_user.*, cbt_user_grup.grup_nama, SUM(`cbt_tes_soal`.`tessoal_nilai`) AS nilai ')
                 ...
                 ->from($this->table)
                 ->join('cbt_user', 'cbt_tes_user.tesuser_user_id = cbt_user.id')
                 ->join('cbt_user_grup', 'cbt_user.user_grup_id = cbt_user_grup.grup_id')
                 ->join('cbt_tes', 'cbt_tes_user.tesuser_tes_id = cbt_tes.tes_id')
                 ->join('cbt_tes_soal', 'cbt_tes_soal.tessoal_tesid = cbt_tes_user.tesuser_tes_id and cbt_tes_soal.tessoal_tesuser_id = cbt_tes_user.tesuser_id')
                 ->group_by('cbt_tes_user.tesuser_id')
                 ->order_by($order);
        return $this->db->get();
    }

    function get_nilai_by_tes_user($tes_id, $id){
        $this->db->select('SUM(`cbt_tes_soal`.`tessoal_nilai`) AS nilai')
                 ->where('(tesuser_tes_id="'.$tes_id.'" AND tesuser_user_id="'.$id.'")')
                 ->from($this->table)
                 ->join('cbt_tes_soal', 'cbt_tes_soal.tessoal_tesid = cbt_tes_user.tesuser_tes_id  AND `cbt_tes_user`.`tesuser_id` = `cbt_tes_soal`.`tessoal_tesuser_id`');
        return $this->db->get();
    }
	
	function get_datatable($start, $rows, $tes_id, $grup_id, $urutkan, $tanggal, $user_id = null, $nama = null){
        $sql = '';
        if($tes_id!='semua'){
            $sql = ' AND tesuser_tes_id="'.$tes_id.'"';
        }
        ...
        if($user_id != null){
            $sql = $sql.' AND tesuser_user_id="'.$user_id.'"';
        }
        if($nama != null){
            $sql = $sql.' AND user_firstname like "%'.$nama.'%"';
        }
		...
		$this->db->select('cbt_tes_user.*,cbt_user_grup.grup_nama, cbt_tes.*, cbt_user.*, SUM(`cbt_tes_soal`.`tessoal_nilai`) AS nilai ')
                 ...
                 ->group_by('cbt_tes_user.tesuser_user_id,`cbt_tes_user`.`tesuser_id`')
				 ->order_by($order)
                 ->limit($rows, $start);
        return $this->db->get();
	}
    
	function get_datatable_count($tes_id, $grup_id, $urutkan, $tanggal, $user_id = null){
        $sql = '';
        if($tes_id!='semua'){
            $sql = ' AND tesuser_tes_id="'.$tes_id.'"';
        }
    ...
    function get_datatable_evaluasi($start, $rows, $tes_id, $urutkan){
        ...
        $this->db->select('cbt_tes_soal.tessoal_id, cbt_tes_soal.tessoal_jawaban_text, cbt_tes.*, cbt_soal.*')
                 ...
                 ->from($this->table)
                 ->join('cbt_tes', 'cbt_tes_user.tesuser_tes_id = cbt_tes.tes_id')
                 ->join('cbt_tes_soal', 'cbt_tes_soal.tessoal_tesid = cbt_tes_user.tesuser_tes_id')
    ...
    function get_datatable_operator($start, $rows, $token){
        $this->db->select('cbt_tes_user.*,cbt_user_grup.grup_nama, cbt_tes.*, cbt_user.*, SUM(`cbt_tes_soal`.`tessoal_nilai`) AS nilai ')
                 ...
                 ->join('cbt_tes', 'cbt_tes_user.tesuser_tes_id = cbt_tes.tes_id')
                 ->join('cbt_tes_soal', 'cbt_tes_soal.tessoal_tesid = cbt_tes_user.tesuser_tes_id')
                 ->group_by('cbt_tes_user.tesuser_tes_id')
