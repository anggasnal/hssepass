...
class Access{
	...
	function login($username, $password){
		...
		if($result){
			...
			if($password === $result->password){
				$this->CI->session->set_userdata('cbt_id',$result->username);
	 ...
	 */
	function is_login(){
		return (($this->CI->session->userdata('cbt_id')) ? TRUE : FALSE);
	}
	...
	function get_username(){
		return $this->CI->session->userdata('cbt_id');
	}
	 ...
	 */
	function logout(){
		$this->CI->session->unset_userdata('cbt_id');
