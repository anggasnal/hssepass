<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed'); 
	class Template{
		...
		function display_tes_back($template, $title, $data=null){
		    $query = $this->_ci->cbt_konfigurasi_model->get_by_kolom_limit('konfigurasi_kode', 'cbt_nama', 1);
		    if($query->num_rows()>0){
		        $data['site_name']=$query->row()->konfigurasi_isi;
		    }else{
		    ...
		    $data['title']=$title;
		    $data['timestamp'] = time();
		    $this->_ci->load->view('template/template_tes_cbt.php',$data);
		}
		...
		function display_login($template, $title, $data=null){
		    //$data['site_name']=$this->_ci->config->item('site_name');
		    $query = $this->_ci->cbt_konfigurasi_model->get_by_kolom_limit('konfigurasi_kode', 'cbt_nama', 1);
		    if($query->num_rows()>0){
		        $data['site_name']=$query->row()->konfigurasi_isi;
		    ...
		    $data['title']=$title;
		    $data['timestamp'] = time();
		    $this->_ci->load->view('template/template_login.php',$data);
		}
		
		function display_home($template, $title, $data=null){
		    //$data['site_name']=$this->_ci->config->item('site_name');
		    $query = $this->_ci->cbt_konfigurasi_model->get_by_kolom_limit('konfigurasi_kode', 'cbt_nama', 1);
		    if($query->num_rows()>0){
		        $data['site_name']=$query->row()->konfigurasi_isi;
		    ...
		    $data['content']=$this->_ci->load->view($template,$data,true);
		    $data['title']=$title;
		    $this->_ci->load->view('template/template_home.php',$data);
		}
