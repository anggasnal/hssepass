...
class Tes_kerjakan extends Tes_Controller {
    ...
    function __construct(){
        ...
        $this->id = $this->cbt_user_model->get_by_kolom_limit('username', $this->username, 1)->row()->id;
	}
    ...
    public function index($tes_id=null){
        if(!empty($tes_id)){
            ...
            $query_tes = $this->cbt_tes_user_model->get_by_user_tes_limit($this->id, $tes_id, 1);
            if($query_tes->num_rows()>0){
                ...
                if($tanggal>=$tanggal_tes){
                    ...
                    $data_soal = $this->get_soal($tessoal->tessoal_id, $query_tes->tes_id);
    ...
    function hentikan_tes(){
        ...
        if($this->form_validation->run() == TRUE){
            $tesid = $this->input->post('hentikan-tes-user-id', TRUE);
            
            $centang = $this->input->post('hentikan-centang', TRUE);
            if(!empty($centang)){
                $data_tes['tesuser_status']=4;
                $this->cbt_tes_user_model->update('tesuser_id', $tesid, $data_tes);
    ...
    function simpan_jawaban(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            $tes_id = $this->input->post('tes-user-id', TRUE);
            ...
            //if($this->cbt_tes_user_model->count_by_status_waktu($tes_id)->row()->hasil>0){
            //
            // revisi 2018-11-15
            // agar waktu mengambil dari waktu php, bukan mysql
            $waktuuser = date('Y-m-d H:i:s');
            if($this->cbt_tes_user_model->count_by_status_waktuuser($tes_id, $waktuuser)->row()->hasil>0){
     ...
     *
     * @param      <type>  $tes_id  The tes user identifier
     */
    function get_tes_info($tes_id=null){
        ...
        if(!empty($tes_id)){
            ...
            if($query_tes->num_rows()>0){
                ...
                $data['data'] = 1;
                $data['tes_id'] = $tes_id;
                $data['tesuser_id'] = $query_tes->tesuser_id;
     ...
     */
    function get_soal_by_tessoal($tessoal_id=null, $tesid=null){
        $data['data'] = 0;
        if(!empty($tessoal_id) AND !empty($tesid)){
            $data_soal = $this->get_soal($tessoal_id, $tesid);
    ...
    private function get_daftar_soal($tes_id=null){
        ...
        $data_soal = '';
        if(!empty($tes_id)){
            $query_tes = $this->cbt_tes_user_model->get_by_user_tes_limit($this->id, $tes_id);
     ...
     */
    private function get_soal($tessoal_id=null, $tesid=null){
        $data['tes_soal_id'] = '';
        $data['tes_soal'] = '';
        $data['data'] = 0;
        if(!empty($tessoal_id) AND !empty($tesid)){
            // Mengecek apakah tes masih berjalan
            // mengambil tesid terus mendapatkan datanya, dicek statusnya dan waktunya
            //if($this->cbt_tes_user_model->count_by_status_waktu($tesid)->row()->hasil>0){
            //
            // revisi 2018-11-15
            // agar waktu mengambil dari waktu php, bukan mysql
            $waktuuser = date('Y-m-d H:i:s');
            if($this->cbt_tes_user_model->count_by_status_waktuuser($tesid, $waktuuser)->row()->hasil>0){
                $data['data'] = 1;
