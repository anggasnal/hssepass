...
class Tes_hasil_detail extends Tes_Controller {
    ...
    public function index($tesid=null){
        $data['url'] = $this->url;

        if(!empty($tesid)){
        	$query_testuser = $this->cbt_tes_user_model->get_by_kolom_limit('tesuser_id', $tesid, 1);
        	if($query_testuser->num_rows()>0){
        		...
        		$query_user = $this->cbt_user_model->get_by_kolom_limit('id', $query_testuser->tesuser_user_id, 1)->row();
        		...
        		$nilai = $this->cbt_tes_soal_model->get_nilai($query_testuser->tesuser_id)->row();
    ...
    function get_datatable(){
		...
		$query = $this->cbt_tes_soal_model->get_datatable($start, $rows, 'soal_detail', $search, $query_testuser->tesuser_id);
		$iFilteredTotal = $query->num_rows();
		
		$iTotal= $this->cbt_tes_soal_model->get_datatable_count('soal_detail', $search, $query_testuser->tesuser_id)->row()->hasil;
