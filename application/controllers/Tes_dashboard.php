...
class Tes_dashboard extends Tes_Controller {
        ...
        if($query_tes->num_rows()>0){
        	...
        	foreach ($query_tes as $tes) {
            	...
            	if($tanggal>$tanggal_tes){
            		...
            		$this->cbt_tes_user_model->update('tesuser_tes_id', $tes->tes_id, $data_tes);
            	}
        	}
        }
    	...
    	if(!empty($tes_id)){
    		...
    		if($query_tes->num_rows()>0){
    			...
    			if($tanggal<$tanggal_tes){
    				...
    				if($this->cbt_tes_user_model->count_by_user_tes($id, $query_tes->tes_id)->row()->hasil==0){
				        ...
				        $data['timestamp'] = strtotime(date('Y-m-d H:i:s'));
				        $data['url'] = $this->url;
				        $data['tes_nama'] = $query_tes->topik_nama." (".$query_tes->tes_lokasi.")";
    ...
    function mulai_tes(){
        ...
        if($this->form_validation->run() == TRUE){
			...
			if($query_tes->num_rows()>0){
				...
				if($this->cbt_tes_user_model->count_by_user_tes($id, $tes_id)->row()->hasil==0){
					...
					if($is_ok==1){
						...
						if($this->cbt_tes_topik_set_model->count_by_kolom('tset_tes_id', $query_tes->tes_id)->row()->hasil>0){
							...
							$data_tes['tesid'] = $tes_id;
							$data_tes['tesuser_user_id'] = $id;
							...
							foreach ($query_subject_set as $subject_set) {
								...
								if($query_soal->num_rows()>0){
									...
									foreach ($query_soal as $soal) {
										...
										$data_soal['tessoal_tesid'] = $tes_id;
	...
	function password(){
        ...
        if($this->form_validation->run() == TRUE){
			...
			if($this->cbt_user_model->count_by_username_password($username, md5($old))>0){
				if($new==$confirm){
					$data['password'] = md5($new);

					$this->cbt_user_model->update('username', $username, $data);
	            ...
	            if($this->cbt_tes_user_model->count_by_user_tes($id, $temp->tes_id)->row()->hasil>0){
	            	...
	            	if($tanggal<$tanggal_tes AND $query_test_user->tesuser_status!=4){
		            	...
		            	if($temp->tes_results_to_users==1){
		            		$record[] = $this->cbt_tes_soal_model->get_nilai($query_test_user->tesuser_id)->row()->hasil." (".($this->cbt_tes_soal_model->get_nilai($query_test_user->tesuser_id)->row()->hasil>=6?"Lulus":"Tidak Lulus").")";
		            	}else{
		            		$record[] = '';
		            	}
