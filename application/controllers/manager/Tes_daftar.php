...
class Tes_daftar extends Member_Controller {
            ...
            if($centang=='1'){
            	foreach( $tes_id as $kunci => $isi ) {
					if($isi=="on"){
						...
						$this->cbt_tes_model->delete('parent', $kunci);
    ...
    function get_datatable_new(){
		...
		if (isset($_GET['sSearch']) && $_GET['sSearch'] != "" ) {
			$search = $_GET['sSearch'];
			if(strtolower($search)=="top management") $search = 3;
			if(strtolower($search)=="line management") $search = 2;
			if(strtolower($search)=="general worker") $search = 1;
		}
		...
		// run query to get user listing
		$query = $this->cbt_tes_model->get_datatable_new($start, $rows, 'tes_nama', $search);
		$iFilteredTotal = $query->num_rows();
		
		$iTotal= $this->cbt_tes_model->get_datatable_count_new('tes_nama', $search)->row()->hasil;
	    ...
	    foreach ($query as $temp) {			
			...
			$record[] = ++$i;
            $record[] = $temp->tes_lokasi;
            $record[] = ($temp->tstgrp_grup_id=="1"? "General Worker":($temp->tstgrp_grup_id=="2"? "Line Management":"Top Management"));
            $record[] = $temp->tes_begin_time;
            $record[] = $temp->tes_end_time;
            $record[] = $temp->tes_duration_time.' Menit';
            $record[] = $temp->tes_score_right;
