...
class Tes_tambah extends Member_Controller {
    ...
    function tambah_tes(){
        ...
        $this->form_validation->set_rules('tambah-lokasi', 'Lokasi Tes','required|strip_tags');
        ...
        if($this->form_validation->run() == TRUE){
            ...
            $query_topik = $this->cbt_topik_model->get_by_kolom('topik_id', $topik_id);
            if($query_topik->num_rows()){
                $query_topik = $query_topik->result();
                foreach ($query_topik as $topik) {
					$data['topik_nama'] = $topik->topik_nama;
                }
            }
            ...
            $data['tes_lokasi'] = $this->input->post('tambah-lokasi', true);
            ...
            $data['tes_begin_time'] = $tanggal[0]." 00:00:00";
            $data['tes_end_time'] = $tanggal[1]." 23:59:59";
			
			$exp0 = explode(" ", $tanggal[0]);
			$exp1 = explode(" ", $tanggal[1]);
            $data['tes_nama'] = $this->input->post('tambah-lokasi', true)." [".$data['topik_nama']."  ".$exp0[0]." - ".$exp1[0]."]";//$this->input->post('tambah-nama', true);
            $cek_nama = 1;
            if(!empty($nama_lama)){
            ...
            if($cek_nama==1 and $this->cbt_tes_model->count_by_kolom('tes_nama', $data['tes_nama'])->row()->hasil>0){
                ...
                if($is_process==1){
                    ...
                    $this->cbt_tes_topik_set_model->delete('tset_tes_id', $tes_id);
                    foreach ($groups as $group) {
                        ...
                        if($group!=0){
							$ddt = array();
							$ddt['tset_tes_id'] = $tes_id;
							$ddt['tset_topik_id'] = $topik_id;
							$ddt['tset_tipe'] = '1';
							$ddt['tset_difficulty'] = $group;
							$ddt['tset_jumlah'] = '10';
							$ddt['tset_jawaban'] = '4';
							$ddt['tset_acak_soal'] = '1';
							$ddt['tset_acak_jawaban'] = '1';
							$this->cbt_tes_topik_set_model->save($ddt);
                    ...
                    $this->cbt_tes_model->delete('parent', $tes_id);
					//$kr = array(8,14,15);
					for($ll =24; $ll<=29 ; $ll++){
						//if(in_array($ll,$kr) && $group==1){
							//do nothing
						//}else{
						$this->tambah_tes_rec($data,$group, $ll, $tes_id);
						//}
					}
	...
	function tambah_tes_rec($data,$group, $topik_id,$parent){
		$tes_id = '';
		$query_topik = $this->cbt_topik_model->get_by_kolom('topik_id', $topik_id);
            if($query_topik->num_rows()){
                $query_topik = $query_topik->result();
				foreach ($query_topik as $topik) {
					$data['topik_nama'] = $topik->topik_nama;
					$data['topik_id'] = $topik->topik_id;
				}
        }
		$data['parent'] = $parent;
        $data['tes_nama'] = $this->input->post('tambah-lokasi', true)." [".$data['topik_nama']."  ".$data['tes_begin_time']." - ".$data['tes_end_time']."]";//$this->input->post('tambah-nama', true);	
        $is_process = 1;
        // Menyimpan data tes
        if(empty($tes_id)){
        	$tes_id = $this->cbt_tes_model->save($data);
