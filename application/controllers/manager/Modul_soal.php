...
class Modul_soal extends Member_Controller {
        	...
        	if(!empty($query_user->opsi1)){
	        	...
	        	foreach ($user_topik as $topik_id) {
	        		...
	        		if($query_topik->num_rows()>0){
	        			...
	        			$counter++;
					$act = ($topik->topik_aktif?"Aktif":"Tidak Aktif");
	        			if(!empty($data['data_topik'])){
        					if($data['data_topik']==$topik->topik_id){
        						$select = $select.'<option value="'.$topik->topik_id.'" selected>'.$act.' - '.$topik->topik_nama.'</option>';
        					}else{
        						$select = $select.'<option value="'.$topik->topik_id.'">'.$act.' - '.$topik->topik_nama.'</option>';	
        					}
        				}else{
        					$select = $select.'<option value="'.$topik->topik_id.'">'.$act.' - '.$topik->topik_nama.'</option>';
        				}
    ...
    function get_datatable(){
	    ...
	    foreach ($query as $temp) {			
			...
			$level = ($temp->soal_difficulty=="1"? "General Worker":($temp->soal_difficulty=="2"? "Line Management":"Top Management"));
			$soal = str_replace("[base_url]", base_url(), $soal);
