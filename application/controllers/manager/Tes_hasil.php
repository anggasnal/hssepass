...
class Tes_hasil extends Member_Controller {
    ...
    function edit_tes(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            $tesid = $this->input->post('edit-testuser-id', TRUE);

            if($pilihan=='hapus'){
                foreach( $tesid as $kunci => $isi ) {
                    if($isi=="on"){
            	...
            	$status['pesan'] = 'Tes berhasil dibuka';
            }else if($pilihan=='hentikan'){
            	foreach( $tesid as $kunci => $isi ) {
                    if($isi=="on"){
                    	$data_tes['tesuser_status']=4;
            			$this->cbt_tes_user_model->update('tesid', $kunci, $data_tes);
                    }
                }
            	$status['status'] = 1;
        ...
        if(!empty($tes_id) AND !empty($grup_id) AND !empty($waktu) AND !empty($urutkan)){
            ...
            if($query->num_rows()>0){
                ...
                foreach ($query as $temp) {
                    ...
                    $worksheet->setCellValueByColumnAndRow(2, $row, $temp->tes_lokasi);
                    $worksheet->setCellValueByColumnAndRow(3, $row, $temp->topik_nama);
                    $worksheet->setCellValueByColumnAndRow(4, $row, $temp->username);
                    $worksheet->setCellValueByColumnAndRow(5, $row, stripslashes($temp->user_firstname));
    ...
    function get_datatable(){
		...
		$query = $this->cbt_tes_user_model->get_datatable($start, $rows, $tes_id, $grup_id, $urutkan, $tanggal, null, $nama);
	    ...
	    foreach ($query as $temp) {			
            ...
            $record[] = ($temp->nilai>=6?$temp->nilai." (LULUS) ":$temp->nilai." (TIDAK LULUS) ");
