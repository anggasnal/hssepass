...
class Tes_hasil_detail extends Member_Controller {
    ...
    public function index($tesid=null){
        ...
        if(!empty($tesid)){
        	...
        	if($query_testuser->num_rows()>0){
        		...
        		$nilai = $this->cbt_tes_soal_model->get_nilai($tesid)->row();
    ...
    function get_datatable(){
		...
		$query = $this->cbt_tes_soal_model->get_datatable($start, $rows, 'soal_detail', $search, $tesid);
		$iFilteredTotal = $query->num_rows();
		
		$iTotal= $this->cbt_tes_soal_model->get_datatable_count('soal_detail', $search, $tesid)->row()->hasil;
