...
class Modul_daftar extends Member_Controller {
    ...
    public function index(){
        ...
        if($query_user->num_rows()>0){
			...
			$act = ($topik->topik_aktif?"Aktif":"Tidak Aktif");
                        $select = $select.'<option value="'.$topik->topik_id.'">'.$act.' - '.$topik->topik_nama.' ['.$jml_soal.']</option>';
                    }
                }
            }else{
    ...
    function get_datatable(){
	    ...
	    foreach ($query as $temp) {			
			...
			$record[] = ($temp->soal_difficulty=="1"? "General Worker":($temp->soal_difficulty=="2"? "Line Management":"Top Management"));
			$soal = $temp->soal_detail;
			$soal = str_replace("[base_url]", base_url(), $soal);
			if(!empty($temp->soal_audio)){
				$posisi = $this->config->item('upload_path').'/topik_'.$temp->soal_topik_id;
