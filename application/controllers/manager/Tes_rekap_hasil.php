...
class Tes_rekap_hasil extends Member_Controller {
    ...
    public function index(){
		...
		$id = $this->users_model->get_login_info($username)->id;
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($query_user->num_rows()>0 AND $query_tes->num_rows()>0){
                ...
                foreach ($query_tes as $tes) {
                	$worksheet->setCellValueByColumnAndRow($kolom, 8, $tes->topik_nama);
                    $kolom++;
                }
            	...
            	foreach ($query_user as $user) {
	                ...
	                foreach ($query_tes as $tes) {
	                	...
	                	$query_nilai = $this->cbt_tes_user_model->get_nilai_by_tes_user($tes->tes_id, $user->id);
